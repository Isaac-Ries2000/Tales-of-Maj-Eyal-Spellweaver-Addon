...
function _M:init(actor)
	...
	Dialog.init(self, _t"Spell Crafting: Slots", game.w * 0.6, game.h * 0.8)
...
Choose an empty spell slot to fill or a used spell slot to replace.
]]}
...
function _M:generateList()
	...
	for j, t in pairs(self.actor.talents_def) do
		if self.actor:knowTalent(t.id) and (t.isKamSpellSlot or t.isKamTeleportSpellSlot) then
			local nodes = talents
	...
	list = {
		{ char='', name=(_t'#{bold}#Choose a Slot#{normal}#'):toTString(), status='', hotkey='', desc=_t"Only Spell Slots can be used.", color=function() return colors.simple(colors.LIGHT_GREEN) end, nodes=talents, shown=true },
		chars = chars,
	}
	self.list = list
