...
function _M:init(actor)
	...
	Dialog.init(self, _t"Spell Crafting: Shapes", game.w * 0.6, game.h * 0.8)
...
Choose a shape.
	...
	local cols = {
		...
		{name=_t"Shapes", width=80, display_prop="name"},
	}
...
function _M:generateList()
	...
	-- Generate list of the shapes
	for j, t in pairs(self.actor.talents_def) do
		if self.actor:knowTalent(t.id) and t.isKamShape then
			local nodes = talents
			local status = tstring{{"color", "LIGHT_GREEN"}, "Shapes"}
	...
	list = {
		{ char='', name=(_t'#{bold}#Choose a Shape#{normal}#'):toTString(), status='', hotkey='', desc=_t"Shapes set how a spell chooses targets.", color=function() return colors.simple(colors.LIGHT_GREEN) end, nodes=talents, shown=true },
		chars = chars,
	}
	self.list = list
