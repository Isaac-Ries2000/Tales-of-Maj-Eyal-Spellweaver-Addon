...
function _M:init(actor)
	...
	Dialog.init(self, _t"Spell Crafting: Teleport Bonuses", game.w * 0.6, game.h * 0.8)
...
Choose an element.
	...
	local cols = {
		...
		{name=_t"Bonuses", width=80, display_prop="name"},
	}
...
function _M:generateList()
	...
	for j, t in pairs(self.actor.talents_def) do
		if self.actor:knowTalent(t.id) and t.isKamTeleportBonus then
			local nodes = talents
			local status = tstring{{"color", "LIGHT_GREEN"}, "Bonuses"}
	...
	list = {
		{ char='', name=(_t'#{bold}#Choose a bonus#{normal}#'):toTString(), status='', hotkey='', desc=_t"Teleport bonuses add an additional effect to the teleport.", color=function() return colors.simple(colors.LIGHT_GREEN) end, nodes=talents, shown=true },
		chars = chars,
	}
	self.list = list
