...
function _M:init(actor)
	...
	Dialog.init(self, _t"Choose a Spell Slot", game.w * 0.6, game.h * 0.8)
...
Choose a spell slot for use in this effect.
]]}
	...
	local cols = {
		...
		{name=_t"Slots", width=80, display_prop="name"},
	}
...
function _M:generateList()
	...
	local canSelectRunes = false
	if self.actor:attr("kam_beacon_of_spellweavers_attr") then
		if self.actor.talents_types["spellweaving/runic-mastery"] then
			canSelectRunes = true
		end
	end

	-- Generate list of the slots
	for j, t in pairs(self.actor.talents_def) do
		if self.actor:knowTalent(t.id) and (not t.kamExcludeFromThreadswitch) and (t.isKamSpellSlot or t.isKamTeleportSpellSlot or (canSelectRunes and (t.type[1] == "inscriptions/runes" or t.type[1] == "inscriptions/taints" or (t.type[1] == "inscriptions/infusions" and KamCalc:isAllowInscriptions(self))))) then
			local nodes = talents
			local status = tstring{{"color", "LIGHT_GREEN"}, "Slots"}
	...
	table.sort(talents, function(s1, s2) 
		if s1.slot and not s2.slot then
			return true
		elseif s2.slot and not s1.slot then
			return false
		elseif s1.slot and s2.slot then
			return s1.slot < s2.slot
		else 
			return s1.cname < s2.cname
		end
	end)
	...
	list = {
		{ char='', name=(_t'#{bold}#Choose a Spell Slot#{normal}#'):toTString(), status='', hotkey='', desc=_t"Only Spell Slots can be used.", color=function() return colors.simple(colors.LIGHT_GREEN) end, nodes=talents, shown=true },
		chars = chars,
	}
	self.list = list
