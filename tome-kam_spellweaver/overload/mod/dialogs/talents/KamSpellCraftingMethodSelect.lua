...
function _M:init(actor)
	...
	Dialog.init(self, _t"Choose a spell crafting method", game.w * 0.6, game.h * 0.8)
...
Choose a spell crafting method to craft a spell with.
]]}
...
function _M:generateList()
	...
	-- Generate list of the crafting methods
	for j, t in pairs(self.actor.talents_def) do
		if self.actor:knowTalent(t.id) and t.isKamSpellCraftingMethod then
			local nodes = talents
			local status = tstring{{"color", "LIGHT_GREEN"}, "Crafting methods"}
	...
	list = {
		{ char='', name=(_t'#{bold}#Choose a Spell Crafting method#{normal}#'):toTString(), status='', hotkey='', desc=_t"Choose the method of spell crafting that you want to use.", color=function() return colors.simple(colors.LIGHT_GREEN) end, nodes=talents, shown=true },
		chars = chars,
	}
	self.list = list
