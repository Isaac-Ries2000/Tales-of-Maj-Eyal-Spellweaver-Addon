...
function _M:init(actor)
	...
	Dialog.init(self, _t"Spell Crafting: Modes", game.w * 0.6, game.h * 0.8)
...
function _M:generateList()
	...
	for j, t in pairs(self.actor.talents_def) do
		if self.actor:knowTalent(t.id) and t.isKamMode then
			local nodes = talents
	...
	list = {
		{ char='', name=(_t'#{bold}#Choose a mode#{normal}#'):toTString(), status='', hotkey='', desc=_t"Spell modes set what a spell does.", color=function() return colors.simple(colors.LIGHT_GREEN) end, nodes=talents, shown=true },
		chars = chars,
	}
	self.list = list
