...
function _M:init(actor)
	...
	Dialog.init(self, _t"Spell Crafting: Shield Modes", game.w * 0.6, game.h * 0.8)
...
function _M:generateList()
	...
	for j, t in pairs(self.actor.talents_def) do
		if self.actor:knowTalent(t.id) and t.isKamShieldMode then
			local nodes = talents
	...
	list = {
		{ char='', name=(_t'#{bold}#Choose a Mode#{normal}#'):toTString(), status='', hotkey='', desc=_t"Shield modes determine the basic functionality of the shield.", color=function() return colors.simple(colors.LIGHT_GREEN) end, nodes=talents, shown=true },
		chars = chars,
	}
	self.list = list
